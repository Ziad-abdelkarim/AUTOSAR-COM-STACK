...
Can_ReturnType Can_Write(Can_HwHandleType Hth, const Can_PduType* PduInfo)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if (MessageObject[CurrentHObj->CanControllerRef->CanControllerId][CurrentHObj->CanObjectId][ObjInd].Confirmation
                        == true)
                {
                    Id = ObjInd;
                    break;
             ...
             */
            MessageObject[CurrentHObj->CanControllerRef->CanControllerId][CurrentHObj->CanObjectId][Id].Confirmation =
                    false;
            /*
            ...
            else
            {
                if (PduInfo->sdu == NULL)
                {
                    ...
                    else
                    {
                         ...
                         */
                        if (MessageObject[CurrentHObj->CanControllerRef->CanControllerId][CurrentHObj->CanObjectId][Id].Confirmation
                                == true)
                        {
                ...
                else
                {
                    ...
                    {
                        // send data
                        MessageObject[CurrentHObj->CanControllerRef->CanControllerId][CurrentHObj->CanObjectId][Id].PduId =
                                PduInfo->swPduHandle;
            ...
            if (Controller == CONTROLLER_ZERO) //check which controllers (Controller0 or Controller1)
            {
                ...
                if (!IntIsEnabled(INT_CAN0)) //Returns TURE if a peripheral interrupt is enabled.
                {
                    ...
                    { //This function enables specific interrupt sources of the CAN controller. Only enabled sources cause a processor interrupt.
                        //CANIntRegister(CAN0_BASE, Can0_InterruptHandler);
                        CANIntEnable(
                                Can.CanConfigSet.CanController[Controller].CanControllerBaseAddress,
                                CAN_INT_MASTER | CAN_INT_ERROR | CAN_INT_STATUS);
 ...
 ***********************************************************************************************/
#if ((Can0TxProcessing==POLLING && Can0RxProcessing==POLLING && Can0BusoffProcessing==POLLING))
    void Can0_InterruptHandler(void)
    {

    }

#elif ((Can0TxProcessing==INTERRUPT || Can0RxProcessing==INTERRUPT || Can0BusoffProcessing==INTERRUPT))
void Can0_InterruptHandler(void)
{
    uint32 ui32Status;
    ...
    else if ((ui32Status >= MIN_OBJECT_HANDLER)
            && (ui32Status <= MAX_OBJECT_HANDLER)) //check if ObjectHandler in range 1-->32 .
    {
        CANIntClear(
                Can.CanConfigSet.CanController[CONTROLLER_ZERO].CanControllerBaseAddress,
                ui32Status);  // Clear the message object interrupt
                            ...
                            ui8NumberOfObject++)
                    {
                                ...
                                == ui32Status) //check which MessageObjectNumber we need to TRANSMIT form it
                        {
                            ...
                            CanIf_TxConfirmation(
                                    MessageObject[CONTROLLER_ZERO][ui8NumberOfObjectHandler][ui8NumberOfObject].PduId);
                        }
                        else
                        {
                            //MISRA
                        ...
                        == RECEIVE)  //check if CanObjectType is receive
                {
                            ...
                            ui8NumberOfObject++)
                    {
                                ...
                                == ui32Status) //check which MessageObjectNumber we need to receive in it
                        {
                            ...
                            ReceiverMailBox.Hoh =
                                    Can.CanConfigSet.CanHardwareObject[ui8NumberOfObjectHandler].CanObjectId;
                            ...
                            CanIf_RxIndication(&ReceiverMailBox,
                                               &ReceiverPduInfo);
                        }
                        else
                        {
 ...
 ***********************************************************************************************/
#if((Can1TxProcessing==POLLING && Can1RxProcessing==POLLING && Can1BusoffProcessing==POLLING))
    void Can1_InterruptHandler(void)
    {

    }

#elif((Can1TxProcessing==INTERRUPT || Can1RxProcessing==INTERRUPT || Can1BusoffProcessing==INTERRUPT))
void Can1_InterruptHandler(void)
{
                        ...
                        == TRANSMIT)  //check if CanObjectType is TRANSMIT
                {
                            ...
                            ui8NumberOfObject++)
                    {
                                ...
                                == ui32Status) //check which MessageObjectNumber we need to TRANSMIT form it
                        {
                             ...
                             */
                            CanIf_TxConfirmation(
                                    MessageObject[CONTROLLER_ONE][ui8NumberOfObjectHandler][ui8NumberOfObject].PduId);
                        }
                        else
                        {
                            //MISRA
 ...
 *******************************************************************************************************************************/
#if (Can0RxProcessing == POLLING || Can1RxProcessing == POLLING)
            ...
            else if (Transition == CAN_T_STOP)
            {
                if (ControllerState[Controller] == CAN_CS_STARTED)
                {
                    ...
                    else if (CanTimeoutDuration != 0)
                    {
                        ...
                        for (HardwareObjectIndex = 0;
                                HardwareObjectIndex < NUMBER_OF_HOH;
                                HardwareObjectIndex++)
                        {
                            if (Can.CanConfigSet.CanHardwareObject[HardwareObjectIndex].CanObjectType
                                    == TRANSMIT
                                            ...
                                            == Controller)
                            {
                                for (ObjectIndex = 0;
                                        ObjectIndex
                                                < Can.CanConfigSet.CanHardwareObject[HardwareObjectIndex].CanHwObjectCount;
                                        ObjectIndex++)
                                {
                                    ...
                                    GET_ADDRESS_VAL(cancontrollerbaseadress,CANIF1CRQ) =
                                            MessageObject[Controller][HardwareObjectIndex][ObjectIndex].MessageObjectNumber;
                                }
                            }
                        }
        #if((Can0TxProcessing==INTERRUPT || Can0RxProcessing==INTERRUPT || Can0BusoffProcessing==INTERRUPT)||(Can1TxProcessing==INTERRUPT || Can1RxProcessing==INTERRUPT || Can1BusoffProcessing==INTERRUPT))
...
void Can_Init(const Can_ConfigType* Config)
{
    ...
    else
    {
        ...
        for (ControllerIndex = 0U; ControllerIndex < NUMBER_OF_CONTROLLERS;
                ControllerIndex++)
        {
        ...
        if (UninitCheck == true)
        {
                    ...
                    ControllerIndex++)
            {
                ...
                if (Config->CanConfigSet.CanController[ControllerIndex].CanControllerActivation)
                {
                    ...
                    SysCtlPeripheralEnable(
                            CanSysCtlPeripheral[ControllerIndex]);
                    ...
                    CANBitClkParms.ui32SyncPropPhase1Seg =
                            (Config->CanConfigSet.CanController[ControllerIndex].CanControllerDefaultBaudrate->CanControllerPropSeg
                                    + Config->CanConfigSet.CanController[ControllerIndex].CanControllerDefaultBaudrate->CanControllerSeg1 + 1U);
                    CANBitClkParms.ui32Phase2Seg =
                            Config->CanConfigSet.CanController[ControllerIndex].CanControllerDefaultBaudrate->CanControllerSeg2;
                    CANBitClkParms.ui32SJW =
                            Config->CanConfigSet.CanController[ControllerIndex].CanControllerDefaultBaudrate->CanControllerSyncJumpWidth;
                    CANBitClkParms.ui32QuantumPrescaler =
                            SysCtlClockGet()
                                    / ((CANBitClkParms.ui32SyncPropPhase1Seg
                                            + CANBitClkParms.ui32Phase2Seg)
                                            * Config->CanConfigSet.CanController[ControllerIndex].CanControllerDefaultBaudrate->CanControllerBaudRate
                                                    * 1000U);
                    // Set bit timing
                    CANBitTimingSet(
                            Config->CanConfigSet.CanController[ControllerIndex].CanControllerBaseAddress,
                            &CANBitClkParms);
                    //CANBitRateSet(CAN0_BASE, SysCtlClockGet(), 200000);
            ...
            // Loop over all HOH
            for (HardwareObjectIndex = 0; HardwareObjectIndex < NUMBER_OF_HOH;
                    HardwareObjectIndex++)
            {
                // Get HOH's Controller ID
                ControllerIndex =
                        Config->CanConfigSet.CanHardwareObject[HardwareObjectIndex].CanControllerRef->CanControllerId;
                // Get HOH's Object Type
                HardwareObjectType =
                        Config->CanConfigSet.CanHardwareObject[HardwareObjectIndex].CanObjectType;

                // Loop over all HTH or HRH
                for (ObjectIndex = 0;
                        ObjectIndex
                                < Config->CanConfigSet.CanHardwareObject[HardwareObjectIndex].CanHwObjectCount;
                        ObjectIndex++)
                {
                    ...
                    MessageObject[ControllerIndex][HardwareObjectIndex][ObjectIndex].Confirmation =
                            true;
                    ...
                    if (HardwareObjectType == RECEIVE)
                    {
                        ...
                        if (Config->CanConfigSet.CanHardwareObject[HardwareObjectIndex].CanIdType
                                == STANDARD)
                        {
                            CANMsgObject.ui32Flags = MSG_OBJ_RX_INT_ENABLE
                                    | MSG_OBJ_USE_ID_FILTER | MSG_OBJ_FIFO;
                        }
                        else if (Config->CanConfigSet.CanHardwareObject[HardwareObjectIndex].CanIdType
                                == EXTENDED)
                        {
                            CANMsgObject.ui32Flags = MSG_OBJ_RX_INT_ENABLE
        ...
        for(INDEX_HOH=0;INDEX_HOH<NUMBER_OF_HOH;INDEX_HOH++)
        {
            if(Can.CanConfigSet.CanHardwareObject[INDEX_HOH].CanControllerRef== &Can.CanConfigSet.CanController[0])
            {
                if(Can.CanConfigSet.CanHardwareObject[INDEX_HOH].CanObjectType==TRANSMIT)
                {
                    ...
                    {
                        if(MessageObject[0][INDEX_HOH][INDEX_MESGOBJ].Confirmation == false)
                        {
        ...
        for(INDEX_HOH=0;INDEX_HOH<NUMBER_OF_HOH;INDEX_HOH++)
        {
            if(Can.CanConfigSet.CanHardwareObject[INDEX_HOH].CanControllerRef== &Can.CanConfigSet.CanController[1])
            {
                if(Can.CanConfigSet.CanHardwareObject[INDEX_HOH].CanObjectType==TRANSMIT)
                {
                    for(INDEX_MESGOBJ=0;INDEX_MESGOBJ<=Can.CanConfigSet.CanHardwareObject[INDEX_HOH].CanHwObjectCount;INDEX_MESGOBJ++)
                    {
                        if(MessageObject[1][INDEX_HOH][INDEX_MESGOBJ].Confirmation == false)
                        {
    ...
    if (CanDriverState == CAN_READY)
    {
        ...
        else
        {
            if (ControllerState[Controller] == CAN_CS_STOPPED)
            {
                ...
                {
                    for (BaudRateIndex = 0; BaudRateIndex < NUMBER_OF_BAUDRATES;
                            BaudRateIndex++)
                    {
                        if (Can.CanConfigSet.CanController[Controller].CanControllerBaudrateConfig[BaudRateIndex].CanControllerBaudRateConfigId
                                == BaudRateConfigID)
                        {
                            ...
                            CanBitTimingParameters.ui32SyncPropPhase1Seg =
                                    Can.CanConfigSet.CanController[Controller].CanControllerBaudrateConfig[BaudRateIndex].CanControllerPropSeg
                                            +\
 Can.CanConfigSet.CanController[Controller].CanControllerBaudrateConfig[BaudRateIndex].CanControllerSeg1
                                            + 1U;
                            ...
                            CanBitTimingParameters.ui32QuantumPrescaler =
                                    SysCtlClockGet()
                                            / ((CanBitTimingParameters.ui32SyncPropPhase1Seg
                                                    + CanBitTimingParameters.ui32Phase2Seg)
                                            *\
 (Can.CanConfigSet.CanController[Controller].CanControllerDefaultBaudrate->CanControllerBaudRate
                                                    * 1000U));
